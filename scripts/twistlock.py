...
def main(argv):
  try:
    ...
    tl_only = os.environ.get('TL_ONLY', False)
    tl_tls_enabled = os.environ.get('TL_TLS_ENABLED', False)
    tl_hash = os.environ.get('TL_HASH', 'sha1')
    tl_incude_package_files = os.environ.get('TL_INCLUDE_PACKAGE_FILES', False)
    tl_upload = os.environ.get('TL_UPLOAD', False)
    tl_details = os.environ.get('TL_DETAILS', False)
    tl_only_fixed = os.environ.get('TL_ONLY_FIXED', False)
    tl_compliance_threshold = os.environ.get('TL_COMPLIANCE_THRESHOLD', False)
    tl_vulnerability_threshold = os.environ.get('TL_VULNERABILITY_THRESHOLD', False)
