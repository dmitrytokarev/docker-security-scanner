...
def main(argv):
  ...
  if tl_only: 
    ...
    proc = subprocess.Popen(twistcli_exec, shell=True, subprocess.stdout=PIPE, subprocess.stderr=PIPE).stdout
    stdout, stderr = proc.communicate()
    if proc.returncode != 0:
      sys.exit(1)
    if cf_metadata:
      print('Printing stdout: ' + proc.stdout)
      tl_report_url = re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', proc.stdout)
