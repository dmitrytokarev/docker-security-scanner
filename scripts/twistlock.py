...
def main(argv):
  ...
  if tl_only == "TRUE": 
    ...
    command = ['/packages/twistcli images scan --address "https://' + tl_console_hostname + ':' + tl_console_port + '" --user "' + tl_console_username + '" --password "' + tl_console_password + '" --include-package-files --hash-method "sha1" ' + docker_image_id]
    proc = subprocess.Popen(command, shell=True)
    stdout, stderr = proc.communicate()
